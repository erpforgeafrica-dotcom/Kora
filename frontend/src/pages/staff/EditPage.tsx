...
export default function EditStaffMembersPage() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { data, update } = useCrud<StaffMember>("/api/staff");
  const { showToast } = useToast();
  const { register, handleSubmit, formState: { errors, isSubmitting }, reset } = useForm<StaffMember>();
  ...
  return (
    <PageLayout title="Edit Staff Member">
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl">
        <FormSection title="Basic Information" description="Update team member details">
